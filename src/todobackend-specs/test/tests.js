...
var url = process.env.URL || 'http://192.168.33.10:7700/todos';
...
describe('Create Todo Item', function(){
  ...
  return assert(item, "body.title").that.equals('Item1';)
  });
...
describe('Update Todo Item', function(){
  ...
  beforeEach(function(done) {
        post(url, {title: 'Item1'}).then(function(res) {
  ...
  it('should have completed set to true after the PATCH update' function() {
...
describe('Delete Todo Item', function(){                                       
  ...
  it('should delete the item' function() {   
        var result = delete(location).then(function (res) {      
        return get(location);                    
  });                     
...
functions update(url, method, data){
